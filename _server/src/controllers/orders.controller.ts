...
export class OrdersController {
  ...
  @ApiQuery({
    name: 'from',
    description: 'Дата начала периода дат посещения',
    required: false,
  })
  @ApiQuery({
    name: 'to',
    description: 'Дата конца периода дат посещения',
    required: false,
  })
  @ApiQuery({
    name: 'status',
  ...
  @ApiQuery({
    name: 'search',
    description: 'Поисковый запрос по ФИО клиента',
    required: false,
  })
  public getOrders(
    ...
    return this.ordersService
      ...
      .sort((a, b) => {
        return Utils.getDateTime(a.createdDate) <
          Utils.getDateTime(b.createdDate)
          ? 1
          : -1;
      });
  }
  ...
  public createOrder(@Body() createOrderDto: CreateOrderDto): OrderDto {
    ...
    if (!createOrderDto.phone) {
      throw new HttpException(
        'Необходимо задать номер телефона',
        HttpStatus.BAD_REQUEST,
      );
    }

    let customer: ICustomerEntity;
    let foundCustomers = this.customerService.query(
      item => item.phone === createOrderDto.phone,
    );
    ...
    if (
      createOrderDto.masterId &&
      !this.staffService.get(createOrderDto.masterId)
    ) {
      throw new HttpException(
        `Мастер по id ${createOrderDto.masterId} не найден`,
    ...
    if (
      ...
      !this.servicesService.get(createOrderDto.serviceId)
    ) {
      throw new HttpException(
        `Услуга по id ${createOrderDto.serviceId} не найдена`,
        HttpStatus.NOT_FOUND,
      );
    }
  ...
  @ApiQuery({
    name: 'finishStatus',
    enum: RecordStatusFinish,
    description: 'Услуга оказана или нет',
  })
  @ApiNotFoundResponse({ description: 'Заявка не найдена' })
  closeOrder(
    @Param('id') id: number,
    @Query('finishStatus') finishStatus: RecordStatusFinish,
  ) {
    const foundOrder = this.ordersService.get(+id);
  ...
  private _getOrderDto(order: IOrderEntity) {
    ...
    if (order.customerId) {
      orderDto.customer = new CustomerDto(
        this.customerService.get(order.customerId),
      );
    }

    if (order.serviceId) {
      orderDto.service = new ServiceDto(
        this.servicesService.get(order.serviceId),
      );
    }
