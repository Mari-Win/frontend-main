...
export class CustomersController {
  ...
  @ApiQuery({
    name: 'search',
    description: 'Фильтрует клиента по ФИО или номер телефона',
    required: false,
  })
  ...
  @ApiBadRequestResponse({
    description: 'Не задано имя, фамилия или номер телефона',
  })
  ...
  createCustomer(@Body() createCustomerDto: CreateCustomerDto): CustomerDto {
    ...
    const existedCustomer = this.customersService.query(
      item => item.phone === createCustomerDto.phone,
    );
